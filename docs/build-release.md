...
版本：0.1.3。GUI 使用 Python 3.12、PySide6；CUDA 运行包使用独立的 CPython 3.10.21。构建不需要运行模型。
...
v0.1.3 随 Release 重新发布组件分片，清单中的 URL、大小和 SHA-256 必须与本次附件逐项一致。安装器通过随附清单获取对应分片，不要求用户先安装旧版软件。第三方依赖版本未变时可以复用经 SHA-256 核验的对应源码归档，并在新 Release 明确标注。
