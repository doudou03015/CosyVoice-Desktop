...
- 文本配音：输入或导入 TXT、DOCX，选择音色和语速，分段生成、试听，导出 WAV/MP3/M4A；参考录音支持 WAV、FLAC、MP3、OGG 和 M4A。
...
v0.1.2 已从产品移除旧的六段参考样本；新安装和升级后都不会显示、自动选择或恢复它们。
龙婉、龙书、龙橙属于本机用户音色，因公开再分发许可未确认，不随 GitHub 安装包提供。请从旧电脑复制
`用户数据目录/voices/local-presets` 到新电脑的同一用户数据目录下的 `voices` 文件夹，或在软件中逐项导入录音。
